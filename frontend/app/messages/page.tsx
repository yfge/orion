            ...
            {items.map((it) => (
              <tr key={it.message_definition_bid} className="border-t">
                ...
                <td className="px-3 py-2 flex gap-2">
                  <Link href={`/messages/${it.message_definition_bid}`} className="text-primary hover:underline">编辑</Link>
                  <button onClick={() => onDelete(it.message_definition_bid)} className="text-red-600 hover:underline">删除</button>
                </td>
              </tr>
            ))}
            {items.length === 0 && (
