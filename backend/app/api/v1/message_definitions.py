...
from ...schemas.message_definitions import MessageDefCreate, MessageDefList, MessageDefOut, MessageDefUpdate
...
def create_message_def(payload: MessageDefCreate, db: Session = Depends(get_db)):
    obj = repo.create_message_def(db, name=payload.name, type=payload.type, schema=payload.schema, status=payload.status or 0)
    db.commit()
    db.refresh(obj)
    return obj
...
def update_message_def(bid: str, payload: MessageDefUpdate, db: Session = Depends(get_db)):
    obj = repo.update_by_bid(db, bid, name=payload.name, type=payload.type, schema=payload.schema, status=payload.status)
    if not obj:
        raise HTTPException(status_code=404, detail="Message definition not found")
    db.commit()
    db.refresh(obj)
