...
from ...schemas.auth_profiles import (
    AuthProfileCreate,
    AuthProfileList,
    AuthProfileOut,
    AuthProfileUpdate,
...
def create_profile(payload: AuthProfileCreate, db: Session = Depends(get_db)):
    obj = repo.create_profile(
        db, name=payload.name, type=payload.type, config=payload.config, status=payload.status or 0
    )
    db.commit()
    db.refresh(obj)
    return obj
...
def update_profile(bid: str, payload: AuthProfileUpdate, db: Session = Depends(get_db)):
    obj = repo.update_by_bid(
        db, bid, name=payload.name, type=payload.type, config=payload.config, status=payload.status
    )
    if not obj:
        raise HTTPException(status_code=404, detail="Auth profile not found")
    db.commit()
