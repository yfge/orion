...
- 端点（Endpoint）：支持 HTTP/MQ/渠道，`adapter_key` + `config` 描述第三方（如 http.feishu_bot、channel.wechat_official_account）
- 派发映射（Dispatch）：消息 → 多端点的映射（双向管理，BID 作为关联）
- 通知 API：`POST /api/v1/notify` 传入消息名或 BID + data 即可自动派发
- 飞书闭环：端点编辑页可直接“测试发送”到飞书机器人 webhook
- 微信公众号通道：通过 `channel.wechat_official_account` 网关支持模板消息发送、重试与指标采集
...
- 端点：
  ...
  - 配置：`app_id`、`app_secret`、`language`（如 `zh_CN`）
- 消息定义：
  - 在“消息定义”中保存一份接近微信模板请求体的“模板”，示例：
    ```json
    {
    ...
    ```
- 派发与调用：
  - 将消息定义绑定到微信端点（简单场景无需额外 mapping）。
  - 业务调用 `/api/v1/notify/` 时传入 `data`（包含 `openid/time/link_url/...`），Orion 会按消息定义渲染模板、合并端点配置，并通过 `wechat_official_account` 网关调用微信公众号模板消息接口。
...
MIT 许可协议——详见根目录 [LICENSE](LICENSE)。
